...
class Carousel {
  constructor(el) {
    this.el = el;
    this.carouselOptions = ['previous', 'play', 'next']; //'add' para añadir un libro
    this.carouselData = [
  ...
  setupCarousel() {
    ...
    this.carouselData.forEach((item, index) => {
      ...
      carouselItem.addEventListener('click', () => {
          // Redirect to corresponding URL when clicked
          window.location.href = item.url || '#'; // Use item.url if available, otherwise default to '#'
      });
    });
